...
const BatchLoadDocuments = ({
    ...
    let fullDocs: Array<IDocumentImport & IFullDocForImport> = []
    ...
    const handleDone = useCallback(async () => {
        if (onAllFullDocsLoaded) {
            await onAllFullDocsLoaded(fullDocs)
        }
        fullDocs = []
    ...
    return (
        <>
            <div className="flex flex-row gap-4 items-center mb-4">
                ...
                {
                    !isRunning && includeRandomSelector && <span className=***REMOVED***flex flex-row justify-end gap-2 text-xs items-center grow***REMOVED***>
                        Randomly select
                        <Input
                        ...
                        <Button
                            disabled={isRunning}
                            onClick={() => {
                                const newSelected = data.slice().sort(() => 0.5 - Math.random()).slice(0, randomSelectCount)
