import type { IDocumentImport, IFullDocForImport, ISearchRecord } from ***REMOVED***@/import/types***REMOVED***
...
export const oikosLayer = async ({
  ...
  const discoveredData =
    doc.data !== null && typeof doc.data === ***REMOVED***object***REMOVED*** ? (doc.data as Record<string, unknown>) : {}
...
export const sensorStation = async ({
  ...
  if (!url && isNaN(Number(uuid)))
    throw new Error(***REMOVED***uuid must be a number - use station id, not uuid***REMOVED***)
  const u =
    url ||
