...
import {
  findEnumProperties,
  removeAllEnumAtPath,
} from ***REMOVED***./select_object_type_for_import/schema_enum_utils***REMOVED***
...
const TypeSetupDialog = ({
  ...
  const [automaticCreateState, setAutomaticCreateState] = useState<***REMOVED***idle***REMOVED*** | ***REMOVED***creating***REMOVED*** | ***REMOVED***error***REMOVED***>(
    ***REMOVED***idle***REMOVED***
  )
...
const ImportAllPage = (): ReactElement => {
  ...
  const [activeView, setActiveView] = useState<***REMOVED***plan***REMOVED*** | ***REMOVED***errors***REMOVED***>(***REMOVED***plan***REMOVED***)
  ...
  const preparationProgress = useMemo(() => {
    const preparingPlans = planList.filter(
      ({ preparationStatus }) => preparationStatus === ***REMOVED***loading***REMOVED***
    )
    return {
          ...
          (
            await fetchRelationships({
              ...
              params: {
                ...
                filters: [
                  {
                    column: ***REMOVED***from_document_uuid***REMOVED***,
                    operator: ***REMOVED***eq***REMOVED***,
                    value: relationship.from_document_uuid,
                  },
                  {
                    column: ***REMOVED***to_document_uuid***REMOVED***,
                    operator: ***REMOVED***eq***REMOVED***,
          ...
          const executed = await executeImportAllSource({
            ...
            persistence: {
              post: (document, signal) =>
                postDocument({ document, token: auth.user?.access_token ?? ***REMOVED******REMOVED***, signal }),
              patch: (uuid, document, signal) =>
                patchDocument({ uuid, document, token: auth.user?.access_token ?? ***REMOVED******REMOVED***, signal }),
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-white">
      <header className="sticky top-0 z-10 flex flex-wrap items-end gap-4 border-b bg-white px-5 py-4">
        <div className="min-w-52 grow">
          ...
          <div className="mt-1 flex flex-wrap gap-4 text-sm text-gray-600">
            ...
            <span
              className={`inline-flex items-center gap-1 ${missingTypes.length > 0 ? ***REMOVED***bg-red-700 px-2 py-1 text-white***REMOVED*** : ***REMOVED***text-green-700***REMOVED***}`}
            >
              {missingTypes.length > 0 ? <X size={14} /> : <Check size={14} />}
              {missingTypes.length} types need setup
            </span>
            <span
              className={`inline-flex items-center gap-1 ${missingSchemas.length > 0 ? ***REMOVED***bg-red-700 px-2 py-1 text-white***REMOVED*** : ***REMOVED***text-green-700***REMOVED***}`}
            >
      ...
      <section className="border-b px-5 py-3" aria-label="Import plan status">
        <div className="flex flex-wrap items-center gap-4 text-sm">
          ...
          <span>
            {relationshipPlans.filter(({ status }) => status === ***REMOVED***ready***REMOVED***).length} relationships
            ready
          </span>
          <span>
            {relationshipPlans.filter(({ status }) => status !== ***REMOVED***ready***REMOVED***).length} relationship
            blockers
          </span>
          {relationshipResults.length > 0 && (
            <>
              <span>
                {relationshipResults.filter(({ status }) => status === ***REMOVED***created***REMOVED***).length} links
                created
              </span>
              <span>
                {relationshipResults.filter(({ status }) => status === ***REMOVED***existing***REMOVED***).length} links
                already existed
              </span>
              <span>
                {relationshipResults.filter(({ status }) => status === ***REMOVED***failed***REMOVED***).length} link
                failures
              </span>
      ...
      <nav className="flex gap-1 border-b px-5 pt-3" aria-label="Import views">
        <Button
          size="xs"
          variant={activeView === ***REMOVED***plan***REMOVED*** ? ***REMOVED***default***REMOVED*** : ***REMOVED***outline***REMOVED***}
          onClick={() => setActiveView(***REMOVED***plan***REMOVED***)}
        >
          Plan
        </Button>
        <Button
          size="xs"
          variant={activeView === ***REMOVED***errors***REMOVED*** ? ***REMOVED***default***REMOVED*** : ***REMOVED***outline***REMOVED***}
          onClick={() => setActiveView(***REMOVED***errors***REMOVED***)}
        >
          Errors ({importErrors.length})
        </Button>
      </nav>

      {activeView === ***REMOVED***errors***REMOVED*** ? (
          ...
          {importErrors.length === 0 ? (
            ...
            <div className="border">
              {importErrors.map((error, index) => (
                <div
                  key={`${error.scope}-${error.recordKey ?? ***REMOVED***source***REMOVED***}-${index}`}
                  ...
                  <div className="mt-3 flex flex-wrap gap-4 text-sm">
                    ...
                    {plan.preparationStatus === ***REMOVED***ready***REMOVED*** && (
                      <>
                        ...
                        <span>
                          {plan.reconciliations.filter(({ status }) => status === ***REMOVED***new***REMOVED***).length} new
                        </span>
                        <span>
                          {plan.reconciliations.filter(({ status }) => status === ***REMOVED***exact***REMOVED***).length}{***REMOVED*** ***REMOVED***}
                          identical
                        </span>
                        <span>
                          {
                        ...
                        <span>
                          {
                            plan.reconciliations.filter(({ status }) => status === ***REMOVED***ambiguous***REMOVED***)
                              .length
                          }{***REMOVED*** ***REMOVED***}
                          ambiguous
                        </span>
                  ...
                  <div className="mt-3 flex flex-wrap gap-3 text-sm">
                    ...
                    {plan.executionResults.length > 0 && (
                      <>
                        <span>
                          {
                            plan.executionResults.filter(({ status }) => status === ***REMOVED***imported***REMOVED***)
                              .length
                          }{***REMOVED*** ***REMOVED***}
                          imported
                        </span>
                        <span>
                          {
                            plan.executionResults.filter(({ status }) => status === ***REMOVED***ignored***REMOVED***)
                              .length
                          }{***REMOVED*** ***REMOVED***}
                          ignored
                        </span>
                        <span>
                          {
                            plan.executionResults.filter(({ status }) => status === ***REMOVED***blocked***REMOVED***)
                              .length
                          }{***REMOVED*** ***REMOVED***}
                          blocked
                        </span>
                        <span>
                          {plan.executionResults.filter(({ status }) => status === ***REMOVED***failed***REMOVED***).length}{***REMOVED*** ***REMOVED***}
                          failed
                        </span>
